...
install_acme(){
    ...
    if [[ -n $(~/.acme.sh/acme.sh -v 2>/dev/null) ]]; then
        ...
        yellow "2. The script could be outdated. Please open up a issue in Github at https://github.com/jafariebi/x_ui/issues"
    fi
    back2menu
}
...
acme_standalone(){
    ...
    if [[ -n $(echo $domainIP | grep nginx) ]]; then
        ...
        if [[ $domainIP != $ipv4 ]] && [[ $domainIP != $ipv6 ]]; then
            ...
            yellow "3. The script could be outdated. Please open up a issue in Github at https://github.com/jafariebi/x_ui/issues"
            exit 1
        fi
    fi
